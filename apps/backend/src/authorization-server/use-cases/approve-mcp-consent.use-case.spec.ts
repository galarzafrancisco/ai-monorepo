...
import { ApproveMcpConsentUseCase } from './approve-mcp-consent.use-case';

describe('ApproveMcpConsentUseCase', () => {
  it('conditionally approves consent and binds the actor with its journey status', async () => {
    ...
    const useCase = new ApproveMcpConsentUseCase(dataSource);

    await useCase.execute('flow-1', 'journey-1', 'actor-1');

    expect(queryBuilder.andWhere).toHaveBeenCalledWith(
      'authorization_code IS NULL',
    );
    expect(queryBuilder.andWhere).toHaveBeenCalledWith('status = :status', {
      status: 'AUTHORIZATION_REQUEST_STARTED',
    });
