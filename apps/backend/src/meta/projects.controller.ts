...
import { ProjectSlugParamsDto } from './dto/project-slug-params.dto';
...
export class ProjectsController {
  ...
  async searchProjects(
    @Query() query: SearchProjectsQueryDto,
  ): Promise<ProjectResponseDto[]> {
  ...
  async getProjectBySlug(
    ...
    const result = await this.projectsService.getProjectBySlug(params.slug);
    if (!result) {
      throw new NotFoundException(
        `Project with slug ${params.slug} not found`,
      );
    }
    return this.mapProjectResultToResponse(result);
  }
  ...
  async getProject(
    ...
    const result = await this.projectsService.getProjectById(params.projectId);
    return this.mapProjectResultToResponse(result);
  }
  ...
  async updateProject(
    ...
    const result = await this.projectsService.updateProject(params.projectId, {
      description: dto.description,
      repoUrl: dto.repoUrl,
    });
    return this.mapProjectResultToResponse(result);
  ...
  @ApiNotFoundResponse({ description: 'Project not found' })
  async deleteProject(@Param() params: ProjectParamsDto): Promise<void> {
    await this.projectsService.deleteProject(params.projectId);
  }
