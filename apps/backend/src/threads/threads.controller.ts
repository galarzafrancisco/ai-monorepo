...
export class ThreadsController {
  ...
  async updateThreadState(
    ...
    const result = await this.threadsService.updateThreadState(params.id, {
      content: dto.content,
  ...
  @RequireScopes(ThreadsScopes.WRITE.id)
  @ApiOperation({ summary: 'Append content to the thread state' })
  @ApiOkResponse({
    type: ThreadStateResponseDto,
  ...
  async appendThreadState(
    ...
    const result = await this.threadsService.appendThreadState(params.id, {
      content: dto.content,
      actorId: user.actorId,
    });
    return result;
