...
const POTENTIALLY_STALE_MESSAGE =
  "We haven't seen activity in a while. If the worker doesn't report back, we'll mark this run as stale and revert the task to its previous state, where another agent will pick it";
...
export function TaskDetailView({
  ...
  const {
    readiness,
    isReady: isChatReady,
    isLoading: isChatReadinessLoading,
  } = useChatReadiness();

  const [liveActivity, setLiveActivity] =
    useState<TaskActivityWireEvent | null>(null);
  const [activityPhase, setActivityPhase] = useState<"idle" | "enter" | "exit">(
    "idle",
  );
  ...
  const handleChangeStatusForPalette = useCallback(
  ...
  useEffect(() => {
    ...
    const statusCommands = Object.entries(TASKS_STATUS).map(
      ([status, info]) => ({
    ...
    const actionCommands = [
      ...
      },
      {
        ...
        description: threadId
          ? "Open this task thread"
          : "Create a thread from this task",
    ...
    task,
    registerCommands,
    handleChangeStatusForPalette,
    handlers,
    showError,
    threadId,
    handleOpenOrCreateThread,
  ]);
  ...
  const [expandedExecutionErrorIds, setExpandedExecutionErrorIds] = useState<
    Set<string>
  >(new Set());
  const [interruptingExecutions, setInterruptingExecutions] = useState<
    Set<string>
  >(new Set());
  ...
  const [respondingToInputRequest, setRespondingToInputRequest] =
    useState<InputRequestResponseDto | null>(null);
  ...
  const handleInterruptExecution = useCallback(
    async (
      executionId: string,
      ...
      if (
        !confirm(
          `Are you sure you want to interrupt the active execution for "${taskName ?? "this task"}"?`,
        )
      ) {
        return;
      }
      ...
      try {
        await ExecutionsService.ActiveTaskExecutionController_interruptExecution(
          { executionId },
        );
  ...
  const saveAnswerToInputRequest = async ({
    answer,
  }: {
    answer: string;
  }): Promise<boolean> => {
  ...
  const loadExecutionsForTask = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      const taskActiveItems = activeResponse.items.map(
        (entry: ActiveTaskExecutionResponseDto): TaskExecutionListItem => ({
          id: `active-${entry.id}`,
      ...
      const taskHistoryItems = historyResponse.items.map(
        (entry: TaskExecutionHistoryResponseDto): TaskExecutionListItem => ({
          id: `history-${entry.id}`,
          executionId: entry.id,
          agentActorId: entry.agentActorId,
          status: entry.status,
    ...
    <div className="task-detail-page">
      {error && (
        <ErrorText>
          <button
            ...
            className="task-detail-page__error-close"
          >
            ×
          </button>
          {error}
      ...
      <DataRowContainer className="task-detail-page__section">
        ...
        >
          ...
          {task.createdByActor.isDeactivated ? (
          ...
          {!task.assigneeActor ? <Text tone="muted">unassigned</Text> : null}
        </DataRow>
        ...
        {task.assigneeActor ? (
          <DataRow
            leading={
              <Avatar
                size={"sm"}
                name={task.assigneeActor.displayName}
                src={task.assigneeActor.avatarUrl || undefined}
              />
            }
          ...
          >
            ...
            {task.assigneeActor.isDeactivated ? (
              <Text as="span" size="1" tone="muted">
                deactivated
              </Text>
            ) : null}
        ...
        <DataRowContainer
          ...
          action={
            dependencyTasks.length > 0 ? (
              ...
              >
                <GitBranch
                  className="task-detail-page__dependency-graph-icon"
                  size={14}
                  strokeWidth={1.5}
                  absoluteStrokeWidth
                />
      ...
      <DataRowContainer className="task-detail-page__section">
        ...
        >
          ...
          <Text>{task.description ? task.description : "no description"}</Text>
        </DataRow>
      </DataRowContainer>

      <DataRowContainer title="Updates" className="task-detail-page__section">
        {timelineItems.length === 0 ? (
          <Text className="task-detail-page__executions-state" tone="muted">
            No updates yet.
          </Text>
        ) : null}
        {visibleTimelineItems.map((item) => {
          if (item.type === "comment") {
            ...
            return (
              <DataRow
                ...
                leading={
                  <Avatar
                    size={"sm"}
                    name={name}
                    src={comment.commenterActor?.avatarUrl || undefined}
                  />
                }
                topRight={
                  <Text size="1" tone="muted">
              ...
              >
                ...
                {comment.commenterActor?.isDeactivated ? (
                  <Text as="span" size="1" tone="muted">
                    deactivated
                  </Text>
                ) : null}
          ...
          const isAssignedToMe =
            user && inputRequest.assignedToActorId === user.actorId;
          const answerText =
            typeof inputRequest.answer === "string"
              ? inputRequest.answer
          ...
          return (
            <DataRow
                ...
                isResolved ? (
                  <div className="task-detail-page__input-request-avatars">
                    <Avatar
                      size={"sm"}
                      name={name}
                      src={askedByActor?.avatarUrl || undefined}
                    />
                    <Avatar
                      size={"sm"}
                      name={answerName}
                      src={assignedToActor?.avatarUrl || undefined}
                    />
                  </div>
                ) : (
                  <Avatar
              ...
              topRight={
                <Text size="1" tone="muted">
                  {elapsedTime(inputRequest.createdAt)}
                </Text>
              }
            >
              <Stack spacing="2">
      ...
      <DataRowContainer className="task-detail-page__section">
        <div className="task-detail-page__activity">
          <div
            className={`task-detail-page__activity-slot ${liveActivity ? "task-detail-page__activity-slot--active" : ""}`}
          >
            <div className="task-detail-page__activity-placeholder">
              <Text size="2" tone="muted">
                Live activity will show here
              </Text>
            </div>
            {liveActivity ? (
              <div
      ...
      <DataRowContainer title="Runs" className="task-detail-page__section">
        ...
        {executionsError ? (
          <Text className="task-detail-page__executions-state" tone="muted">
            Failed to load runs: {executionsError}
          </Text>
        ) : null}
        ...
        {visibleExecutions.map((execution) => {
          ...
          const actorName =
            actor?.displayName ?? shortId(execution.agentActorId);
          const actorSlug = actor?.slug;
          const statusTag = getExecutionStatusTag(execution.status);
          const hasFailureDetails =
            execution.status === "FAILED" &&
            Boolean(execution.errorCode || execution.errorMessage);
          const isFailureDetailsExpanded = expandedExecutionErrorIds.has(
            execution.id,
          );
          const isActive = execution.source === "active";
          const isPotentiallyStale =
            isActive && isExecutionPotentiallyStale(execution, stalenessNow);
          const isInterrupting = interruptingExecutions.has(
              ...
              <Stack spacing="1">
                  ...
                  <span className="task-detail-page__execution-meta">
                    ...
                    {hasFailureDetails ? (
                      <button
                        ...
                        onClick={() =>
                          toggleExecutionErrorDetails(execution.id)
                        }
                        aria-label={
                          isFailureDetailsExpanded
                ...
                <Text as="span" size="1" tone="muted">
                  <span className="task-detail-page__execution-stats">
                    ...
                    <span className="task-detail-page__execution-stats-separator">
                      |
                    </span>
                    session:{" "}
                    {execution.runnerSessionId
                      ? shortId(execution.runnerSessionId)
                      : "pending"}
                    {isActive ? (
                      <>
                        <span className="task-detail-page__execution-stats-separator">
                  ...
                  <div className="task-detail-page__execution-error-detail">
                    {execution.errorCode ? (
                      <Text
                        ...
                        size="1"
                        style="mono"
                        className="task-detail-page__execution-error-code"
                      >
                        {execution.errorCode}
                      </Text>
                    ) : null}
      ...
      {!task.assigneeActor && (
        <DataRowContainer className="task-detail-page__comment-buttons">
          <Button size="lg" onClick={() => setShowAssignPop(true)}>
            Assign
          </Button>
          <Button
            size="lg"
      ...
      {showAssignPop ? (
        <ActorSearchPop onCancel={cancelAssignment} onSave={saveAssignment} />
      ) : null}
      {showTagPop ? (
        <TagSearchPop
          onCancel={cancelTag}
...
export function TaskDetailPage() {
  ...
  useEffect(() => {
    ...
    getTaskById(taskId)
      ...
      .finally(() => {
        if (!cancelled) setIsFetchingTask(false);
      });
  ...
  const isLoadingTask =
    !task && (!hasLoadedOnce || isLoading || isFetchingTask);
  ...
  const handlers: TaskDetailHandlers = {
    ...
    changeStatus: ({ taskId, status }) =>
      TasksService.TasksController_changeStatus({
        id: taskId,
        body: { status },
      }),
    addTag: ({ taskId, tag }) =>
      TasksService.TasksController_addTagToTask({
        id: taskId,
        body: { name: tag.name },
      }),
    removeTag: ({ taskId, tagId }) =>
      TasksService.TasksController_removeTagFromTask({ id: taskId, tagId }),
    addDependency: async ({ taskId, dependencyTaskId }) => {
      const currentTask = allCachedTasks.find((t) => t.id === taskId);
      if (!currentTask) return;
      const updatedDependsOnIds = [
        ...(currentTask.dependsOnIds || []),
        dependencyTaskId,
      ];
      ...
      try {
        ...
        console.error(
          `Failed to hydrate dependency ${dependencyTaskId} after adding`,
          err,
        );
      }
    },
    removeDependency: async ({ taskId, dependencyTaskId }) => {
      const currentTask = allCachedTasks.find((t) => t.id === taskId);
      if (!currentTask) return;
      const updatedDependsOnIds = (currentTask.dependsOnIds || []).filter(
...
function isExecutionPotentiallyStale(
  execution: TaskExecutionListItem,
  now: number,
): boolean {
  return (
    now - new Date(execution.lastActivityAt).getTime() >=
    POTENTIALLY_STALE_THRESHOLD_MS
  );
}
